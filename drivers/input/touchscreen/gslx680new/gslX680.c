...
static void glsX680_monitor_events(struct work_struct *work)
{
	if (!_check_chip_state(glsX680_i2c) || gTestMonitor == 1) {
		...
		printk("gsl_monitor_timer_handle check success!!!\n");
...
static unsigned long idle_data = 0;
...
static ssize_t gsl_idle_enable_store_l(const char *buf)
{
	...
	error = kstrtoul(buf, 10, &next_state);
	if (error)
		return error;
	if (next_state != idle_data) {
		idle_data = next_state;
