...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval != 0) {
		FTS_ERROR("%s() - ERROR: Could not convert the given input to a number. The given input was: \"%s\"", __func__, buf);
		goto error_return;
	}
#if FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy(1);
...
*/
static DEVICE_ATTR(fts_fw_update, 0644, fts_fwupdate_show, fts_fwupdate_store);
...
*/
static DEVICE_ATTR(fts_upgrade_app, 0644, fts_fwupgradeapp_show, fts_fwupgradeapp_store);
...
static DEVICE_ATTR(fts_show_log, 0644, fts_show_log_show, fts_show_log_store);
static DEVICE_ATTR(fts_module_config, 0644, fts_module_config_show, fts_module_config_store);
...
#if FTS_ESDCHECK_EN
static DEVICE_ATTR(fts_esd_check, 0644, fts_esdcheck_show, fts_esdcheck_store);
#endif
