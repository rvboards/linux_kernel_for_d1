...
unsigned char FT3C47_TestItem_ForceTouch_SCapCbTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		for (index = m_iAllRx + g_ScreenSetParam.iTxNum; index < m_iAllTx + m_iAllRx; ++index) {
			m_RawData[0 + g_ScreenSetParam.iTxNum][m_iForceTouchRx + index - m_iAllRx - g_ScreenSetParam.iTxNum] = m_ucTempData[index];
		}
		...
		for (index = g_ScreenSetParam.iRxNum; index < m_iAllRx; ++index) {
			m_RawData[1 + g_ScreenSetParam.iTxNum][index - g_ScreenSetParam.iRxNum] = m_ucTempData[index];
		}
		for (index = m_iAllRx + g_ScreenSetParam.iTxNum; index < m_iAllTx + m_iAllRx; ++index) {
			m_RawData[1 + g_ScreenSetParam.iTxNum][m_iForceTouchRx + index - m_iAllRx - g_ScreenSetParam.iTxNum] = m_ucTempData[index];
		}
