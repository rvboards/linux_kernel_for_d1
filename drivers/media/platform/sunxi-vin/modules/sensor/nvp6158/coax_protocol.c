...
{
	NC_VD_ACP_CMDDEF_STR *pRet = &coax_nvp6158_cmd_lists[def];
...
{
	NC_VD_COAX_Init_STR *pRet = &coax_nvp6158_init_lists[def];
...
{
	NC_VD_COAX_Init_STR *pRet = &coax_nvp6158_acp_16bit_init_lists[def];
...
static int __NC_VD_COAX_16bit_Command_Copy(NC_FORMAT_STANDARD format, NC_VIVO_CH_FORMATDEF vivofmt, unsigned char *Dst, NC_VD_ACP_CMDDEF_STR *pCMD)
{
	...
	if ((vivofmt == AHD20_720P_25P) || (vivofmt == AHD20_720P_30P) ||\
		...
		sensor_dbg("[drv_coax] Can not send commands!! Unsupported format!!\n");
		return 0;
	}
...
* ioctl : IOC_VDEC_COAX_TX_INIT
***************************************************************************************/
void coax_nvp6158_tx_init(nvp6158_coax_str *ps_coax_str)
{
	...
	sensor_dbg("[drv]tx_src:            5x7C>> 0x%02X\n", CoaxVal->rx_src);
	sensor_dbg("[drv]tx_slice_lev:      5x7D>> 0x%02X\n", CoaxVal->rx_slice_lev);
	sensor_dbg("[drv]tx_pel_baud:       3x02>> 0x%02X\n", CoaxVal->tx_baud[distance]);
	sensor_dbg("[drv]tx_pel_line_pos0:  3x07>> 0x%02X\n", CoaxVal->tx_pel_line_pos0[distance]);
	sensor_dbg("[drv]tx_pel_line_pos1:  3x08>> 0x%02X\n", CoaxVal->tx_pel_line_pos1[distance]);
	sensor_dbg("[drv]tx_mode:           3x0B>> 0x%02X\n", CoaxVal->tx_mode);
	sensor_dbg("[drv]tx_baud:           3x00>> 0x%02X\n", CoaxVal->tx_baud[distance]);
	sensor_dbg("[drv]tx_line_pos0:      3x03>> 0x%02X\n", CoaxVal->tx_line_pos0[distance]);
	sensor_dbg("[drv]tx_line_pos1:      3x04>> 0x%02X\n", CoaxVal->tx_line_pos1[distance]);
	sensor_dbg("[drv]tx_line_count:     3x05>> 0x%02X\n", CoaxVal->tx_line_count);
	sensor_dbg("[drv]tx_line_count_max: 3x0A>> 0x%02X\n", CoaxVal->tx_line_count_max);
	sensor_dbg("[drv]tx_sync_pos0:      3x0D>> 0x%02X\n", CoaxVal->tx_sync_pos0[distance]);
	sensor_dbg("[drv]tx_sync_pos1:      3x0E>> 0x%02X\n", CoaxVal->tx_sync_pos1[distance]);
	sensor_dbg("[drv]tx_even:           3x2F>> 0x%02X\n", CoaxVal->tx_even);
	sensor_dbg("[drv]tx_zero_length:    3x0C>> 0x%02X\n", CoaxVal->tx_zero_length);
#endif
...
***************************************************************************************/
void coax_nvp6158_tx_cmd_send(nvp6158_coax_str *ps_coax_str)
/* void coax_tx_cmd_send(const unsigned char txch,  NC_VIVO_CH_FORMATDEF fmt_def , NC_FORMAT_STANDARD vformat, NC_COAX_CMD_DEF txcmd) */
{
	/* NC_VD_COAX_STR *coax_tx = (NC_VD_COAX_STR*)p_param; */
	int i;
	int cmd_cnt = 0;
	...
	sensor_dbg("[drv_coax]Ch: %d Command >>>>> %s >>> autostop = %d\n", ch, pCMD->name, pCMD->autostop);
	...
	/*
	sensor_dbg("tx_bank[%2x], tx_cmd_addr[%2x], tx_shot_addr[%2x]\n", tx_bank, tx_cmd_addr, tx_shot_addr);
	for (i=0;i<cmd_cnt;i++)
	sensor_dbg("[%2x] ",  command[i]);
	sensor_dbg("\n ");
	 fill command + shot
	*/
	if (format == FMT_SD) {
...
***************************************************************************************/
void coax_nvp6158_rx_init(nvp6158_coax_str *ps_coax_str)
{
...
***************************************************************************************/
void coax_nvp6158_rx_data_get(nvp6158_coax_str *coax_rx)
{
	/* NC_VD_COAX_STR *coax_rx = (NC_VD_COAX_STR*)p_param; */
...
***************************************************************************************/
void coax_nvp6158_rx_buffer_clear(nvp6158_coax_str *ps_coax_str)
{
...
***************************************************************************************/
void coax_nvp6158_rx_deinit(nvp6158_coax_str *ps_coax_str)
{
	/* NC_VD_COAX_STR *coax_val = (NC_VD_COAX_STR*)p_param; */
...
***************************************************************************************/
void coax_nvp6158_fw_ready_cmd_to_isp_send(void *p_param) // FW Ready
{
	int ch = 0;
	int devnum = 0;
	int ret = FW_FAILURE;
...
***************************************************************************************/
void coax_nvp6158_fw_ready_cmd_ack_from_isp_recv(void *p_param)
{
	int ret = FW_FAILURE;
	int ch = 0;
	int devnum = 0;
...
***************************************************************************************/
void coax_nvp6158_fw_start_cmd_to_isp_send(void *p_param)
{
	int ch = 0;
	int devnum = 0;
...
***************************************************************************************/
void coax_nvp6158_fw_start_cmd_ack_from_isp_recv(void *p_param)
{
	int ch = 0;
	int devnum = 0;
	int ret = FW_FAILURE;
	...
	if (gpio_i2c_read(nvp6158_iic_addr[devnum], 0x57 + ((ch%2)*0x80)) == 0x02) {
		if (gpio_i2c_read(nvp6158_iic_addr[devnum], 0x56 + ((ch%2)*0x80)) == 0x00) {
			sensor_dbg(">>>>> DRV[%s:%d]CH:%d Receive ISP status : [START]\n", __func__, __LINE__, ch);
			ret = FW_SUCCESS;
		} else {
...
***************************************************************************************/
void coax_nvp6158_fw_one_packet_data_to_isp_send(void *p_param)
{
	int ch = 0;
	int devnum = 0;
	int ii = 0;
...
***************************************************************************************/
void coax_nvp6158_fw_one_packet_data_ack_from_isp_recv(void *p_param)
{
	int ret = FW_FAILURE;
	int ch = 0;
	int devnum = 0;
	...
	if (gpio_i2c_read(nvp6158_iic_addr[devnum], 0x57 + ((ch%2)*0x80)) == 0x02) {
		...
		if (pstFileInfo->currentpacketnum == 0) {
			if (gpio_i2c_read(nvp6158_iic_addr[devnum], 0x56 + ((ch%2)*0x80)) == 0x03) {
				...
				sensor_dbg(">>>>> DRV[%s:%d] CH:%d, Failed, error status, code=3..................\n", __func__, __LINE__, ch);
				return;
			}
		}
...
***************************************************************************************/
void coax_nvp6158_fw_end_cmd_to_isp_send(void *p_param)
{
	int ch = 0;
	int devnum = 0;
	int send_success = 0;
...
***************************************************************************************/
void coax_nvp6158_fw_end_cmd_ack_from_isp_recv(void *p_param)
{
	int ch = 0;
	int devnum = 0;
...
***************************************************************************************/
void coax_nvp6158_option_rt_nrt_mode_change_set(void *p_param)
{
	NC_VD_COAX_STR *coax_val = (NC_VD_COAX_STR *)p_param;
...
* ioctl : IOC_VDEC_COAX_TEST_TX_INIT_DATA_READ
***************************************************************************************/
void coax_nvp6158_test_tx_init_read(NC_VD_COAX_TEST_STR *coax_tx_mode)
{
...
***************************************************************************************/
void coax_nvp6158_test_data_set(NC_VD_COAX_TEST_STR *coax_data)
{
	unsigned char temp_reg;
	sensor_dbg("[DRV_Set]bank(0x%02X)/addr(0x%02X)/param(0x%02X)\n", coax_data->bank, coax_data->data_addr, coax_data->param);
...
***************************************************************************************/
void coax_nvp6158_test_data_get(NC_VD_COAX_TEST_STR *coax_data)
{
	gpio_i2c_write(nvp6158_iic_addr[coax_data->chip_num], 0xFF, coax_data->bank);
	coax_data->param = gpio_i2c_read(nvp6158_iic_addr[coax_data->chip_num], coax_data->data_addr);
	sensor_dbg("[DRV_Get]bank(0x%02X), addr(0x%02X), param(0x%02X)\n", coax_data->bank, coax_data->data_addr, coax_data->param);
}
...
* ioctl : IOC_VDEC_COAX_TEST_DATA_READ
***************************************************************************************/
void coax_nvp6158_test_Bank_dump_get(NC_VD_COAX_BANK_DUMP_STR *coax_data)
{
	int ii = 0;
